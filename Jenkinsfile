    ...
    stages {
        ...
        stage("Containerize") {
            steps {
                ...
                sh  "docker build -t ${AWS_ACCOUNT_ID}.dkr.ecr.${AWS_REGION}.amazonaws.com/aline-underwriter-kl:latest . --no-cache"
            }
        }
        stage("AWS Login") {
            steps {
        ...
        stage("Push to Repository") {
            steps {
                sh "docker push ${AWS_ACCOUNT_ID}.dkr.ecr.${AWS_REGION}.amazonaws.com/aline-underwriter-kl:latest"
            }
        }
